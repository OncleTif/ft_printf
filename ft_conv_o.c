...
/*   Updated: 2016/01/27 14:36:07 by tmanet           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_conv_o(t_str_conv *sub, va_list ap)
{
	if (ft_strequ("ll", sub->type) || (ft_strequ("j", sub->type)))
		sub->str_out = ft_ulltoa_base(va_arg(sub->ap, unsigned long long), 8, 'a');
		...
		sub->str_out = ft_uitoa_base((unsigned char)va_arg(sub->ap, unsigned int), 8, 'a');
	else if (ft_strequ("h", sub->type))
		sub->str_out = ft_uitoa_base((unsigned short)va_arg(sub->ap, unsigned int), 8, 'a');
	else
