...
/*   Updated: 2016/01/27 14:33:50 by tmanet           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_conv_u(t_str_conv *sub, va_list ap)
{
	if (ft_strequ("ll", sub->type) || (ft_strequ("j", sub->type)))
		sub->str_out = ft_ulltoa_base(va_arg(sub->ap, unsigned long long), 10, 'a');
		...
		sub->str_out = ft_uitoa_base((unsigned char)va_arg(sub->ap, unsigned int),
				10, 'a');
	else
